  ...
  <button type="button" id="closeButton" onClick={() => (location.href = '/')} className="absolute top-3 right-3 text-red-600 text-2xl leading-none">&times;</button>
        ...
        <form id="loginForm" className="space-y-4" onSubmit={handleSubmit}>
          <div>
            ...
            <input name="email" id="email" value={email} onChange={e => setEmail(e.target.value)} type="email" placeholder="you@example.com" aria-describedby="emailErr" className="w-full mt-1 p-2 border rounded focus:outline-none focus:ring focus:border-blue-500" required />
            <p className={`${errors.email ? 'block' : 'hidden'} text-sm text-red-600 mt-1`} id="emailErr">Please enter a valid email.</p>
          </div>

          <div className="relative">
            <label htmlFor="password" className="block text-gray-700">Password</label>
            <input name="password" id="password" value={password} onChange={e => setPassword(e.target.value)} type={showPass ? 'text' : 'password'} placeholder="Enter your password" aria-describedby="passwordErr" className="w-full mt-1 p-2 border rounded pr-10 focus:outline-none focus:ring focus:border-blue-500" required />
            <button type="button" aria-label={showPass ? 'Hide password' : 'Show password'} onClick={() => setShowPass(s => !s)} className={`bx ${showPass ? 'bx-show' : 'bx-hide'} absolute right-3 top-9 text-gray-500 show-hide`}></button>
            <p className={`${errors.pass ? 'block' : 'hidden'} text-sm text-red-600 mt-1`} id="passwordErr">At least 8 chars, 1 uppercase, 1 lowercase, 1 number, 1 symbol.</p>
          </div>

          <div className="flex items-center justify-between text-sm text-gray-600">
            <label className="flex items-center">
              <input type="checkbox" id="remember_me" checked={remember} onChange={e => setRemember(e.target.checked)} className="mr-2 accent-blue-600" /> Remember me
            </label>

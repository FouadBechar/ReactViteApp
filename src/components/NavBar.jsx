...
export default function NavBar() {
  ...
  const micBtn = $("#micBtn");
    ...
    function closeMenu() {
      ...
      } catch (err) { console.debug('NavBar focus restore error', err); }
    ...
    } catch (err) { history = []; console.debug('NavBar load history error', err); }
    const MAX_HISTORY = 6;
  ...
  .catch((err) => { console.debug('NavBar fetch pages.json error', err); });

    function setHistory(arr) {
      history = arr;
      try {
        localStorage.setItem("searchHistory", JSON.stringify(history));
      } catch (err) { console.debug('NavBar setHistory persist error', err); }
    }
    ...
    return () => {
      ...
      } catch (err) { console.debug('NavBar cleanup __navRefactor error', err); }
      try {
        if (window.handleGoogleSuggestions)
          delete window.handleGoogleSuggestions;
      } catch (err) { console.debug('NavBar cleanup handleGoogleSuggestions error', err); }
    };
            ...
            <li className="has-dropdown">
              ...
              >
                <ul>
                  ...
                  <li>
                    <a href="https://www.totalav.com/free-antivirus-software-2">
                      Protect PC
                    </a>
                  </li>
                </ul>
